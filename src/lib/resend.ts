...
export async function sendWelcomeEmail(email: string, name?: string | null) {
  ...
  await resend.emails.send({
    from: getFromEmail(),
    to: email,
    subject: `Welcome to ${BRAND_NAME} ✦ Your Ritual Begins`,
    html: `
      <div style="font-family: serif; max-width: 600px; margin: 0 auto; padding: 40px; color: #1a1a1a; background-color: #faf9f6;">
        <h1 style="font-size: 28px; font-weight: normal; letter-spacing: 0.2em; text-align: center; text-transform: uppercase; margin-bottom: 40px;">${BRAND_NAME.toUpperCase()}</h1>
        <div style="line-height: 1.8; font-size: 15px;">
          <p>Welcome home, ${displayName},</p>
          <p>We are honored to have you join our community. Your ${BRAND_NAME} profile is now complete, and your journey toward intentional, beautiful rituals has officially begun.</p>
          ...
          <p style="margin-top: 40px; border-top: 1px solid #e5e5e5; pt: 20px; font-size: 13px; color: #999; text-align: center;">
            With gratitude,<br/>The ${BRAND_NAME} Team
          </p>
        </div>
      </div>
    `,
  ...
  await resend.emails.send({
    ...
    html: `
      <div style="font-family: serif; max-width: 600px; margin: 0 auto; padding: 40px; color: #1a1a1a;">
        <h1 style="font-size: 24px; font-weight: normal; letter-spacing: 0.1em; text-align: center; text-transform: uppercase;">${BRAND_NAME.toUpperCase()}</h1>
        <div style="margin-top: 40px; line-height: 1.6;">
          <p>Hello ${displayName},</p>
          <p>Thank you for joining the journey with us. To begin your personalized ${BRAND_NAME} experience, please verify your email address using the code below.</p>
          ...
          <p style="margin-top: 40px;">With gratitude,<br/>The ${BRAND_NAME} Team</p>
        </div>
      </div>
    `,
  });
